    ...
    **kwargs: Additional keyword arguments to customize the plot.
    ...
    if plot_type == 'countplot':
        ...
        if plot_type == 'bar':
            ...
            for bar in bars:
                width = bar.get_width()
                ax.text(width, bar.get_y() + bar.get_height() / 2, '%.2f' % width, ha='right', va='center')
...
def weighted_kappa_manual(y1, y2, labels, weights='additive'):
    # Create the confusion matrix
    confusion = confusion_matrix(y1, y2, labels=labels)
    
    # Number of ratings
    n_ratings = len(confusion)

    # Create the weight matrix
    if weights == 'linear':
        weight_matrix = np.abs(np.subtract.outer(np.arange(n_ratings), np.arange(n_ratings)))
    elif weights == 'quadratic':
        weight_matrix = np.square(np.subtract.outer(np.arange(n_ratings), np.arange(n_ratings)))
    else:
        raise ValueError("Unsupported weight type. Use 'linear' or 'quadratic'.")

    # Observed agreement
    observed_agreement = np.sum(confusion * (1 - weight_matrix / (n_ratings - 1))) / np.sum(confusion)
    return observed_agreement

def calculate_brennan_prediger_kappa(rater1, rater2, categories):
    """
...
def calculate_statistics(df: pd.DataFrame, col1: str, col2: str, all_ratings: List[int] = [1, 2, 3, 4, 5]) -> dict:
    ...
    max_diff = max(all_ratings) - min(all_ratings)
    abs_mean_diff = np.mean(np.abs(df[col1] - df[col2]))
    rel_abs_mean_diff = 1 - abs_mean_diff / max_diff

    # Calculate Weighted Kappa
    kappa = cohen_kappa_score(df[col1].round(0), df[col2].round(0), labels=all_ratings, weights='quadratic')

    observed_kappa = weighted_kappa_manual(df[col1].round(0), df[col2].round(0), labels=all_ratings, weights='quadratic')

    bag_s_value = bag_s(df[col1].round(0), df[col2].round(0), 5)

    brennan_prediger_kappa = calculate_brennan_prediger_kappa(df[col1].round(0), df[col2].round(0), all_ratings)
    ...
    statistics = {
        ...
        "Observed Kappa": observed_kappa,
        "Absolute Mean Difference": abs_mean_diff,
        "Relative Absolute Mean Difference": rel_abs_mean_diff,
        "BAG's S": bag_s_value,
